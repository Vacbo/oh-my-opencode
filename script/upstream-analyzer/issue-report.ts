...
function buildFooter(config: AnalyzerConfig): string {
  return [
    ...
    "",
    `_Classify chain: \`${describeChain(config.classifyChain)}\`_`,
    `_Slop-verify chain: \`${describeChain(config.slopVerifyChain)}\`_`,
    `_Synthesis chain: \`${describeChain(config.synthesisChain)}\`_`,
  ].join("\n")
}
