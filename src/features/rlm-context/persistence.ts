...
async function validateBlobIntegrity(sessionDir: string, manifest: PersistedVariableInfo[]): Promise<void> {
  for (const varInfo of manifest) {
    ...
    if (fileStat.size !== varInfo.byteSize) {
      throw new Error(`Blob size mismatch for ${varInfo.name}: expected ${varInfo.byteSize}, got ${fileStat.size}`)
    }
  }
}

async function quarantineSession(storageDir: string, sessionId: string, error: unknown): Promise<void> {
  try {
