...
describe("createSkillContext", () => {
  ...
  it("skips Claude skill discovery when claude_code.skills.claude is false", async () => {
    // given
    const discoverConfigSourceSkillsSpy = spyOn(
      skillLoader,
      "discoverConfigSourceSkills",
    ...
    const pluginConfig = OhMyOpenCodeConfigSchema.parse({
      claude_code: { skills: { claude: false, agents: true } },
    });

    try {
      // when
      await createSkillContext({ directory: testDirectory, pluginConfig });

      // then
      expect(discoverUserClaudeSkillsSpy).not.toHaveBeenCalled();
      expect(discoverProjectClaudeSkillsSpy).not.toHaveBeenCalled();
      expect(discoverProjectAgentsSkillsSpy).toHaveBeenCalledWith(
        testDirectory,
      );
  ...
  it("skips .agents skill discovery when claude_code.skills.agents is false", async () => {
    // given
    const discoverConfigSourceSkillsSpy = spyOn(
      skillLoader,
      "discoverConfigSourceSkills",
    ...
    const pluginConfig = OhMyOpenCodeConfigSchema.parse({
      claude_code: { skills: { claude: true, agents: false } },
    });
    ...
    try {
      ...
      // then
      expect(discoverUserClaudeSkillsSpy).toHaveBeenCalled();
      expect(discoverProjectClaudeSkillsSpy).toHaveBeenCalledWith(
        testDirectory,
      );
