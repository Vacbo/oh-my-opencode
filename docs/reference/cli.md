...
Complete reference for the published `@vacbo/oh-my-opencode` package and its `oh-my-opencode` CLI binary. During the rename transition, OpenCode plugin registration now prefers `oh-my-openagent` inside `opencode.json`.

## Basic Usage

```bash
# Display help
bunx @vacbo/oh-my-opencode

# Or with npx
npx @vacbo/oh-my-opencode
```
...
5. **Telemetry Defaults**: Anonymous telemetry stays disabled unless you explicitly opt in through environment variables
...
Anonymous telemetry uses PostHog with a hashed installation identifier. To opt in, set `OMO_SEND_ANONYMOUS_TELEMETRY=1` and `POSTHOG_API_KEY=<your project key>`. You can still force-disable it with `OMO_DISABLE_POSTHOG=1`. See [Privacy Policy](../legal/privacy-policy.md).
...
```bash
bunx @vacbo/oh-my-opencode run <message>
```
...
```bash
bunx @vacbo/oh-my-opencode get-local-version
```
...
```bash
bunx @vacbo/oh-my-opencode version
```
...
bunx @vacbo/oh-my-opencode mcp oauth login <server-name> --server-url https://api.example.com

# Login with explicit client ID and scopes
bunx @vacbo/oh-my-opencode mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes read write

# Remove stored OAuth tokens
bunx @vacbo/oh-my-opencode mcp oauth logout <server-name> --server-url https://api.example.com

# Check OAuth token status
bunx @vacbo/oh-my-opencode mcp oauth status [server-name]
```
...
**Naming Note**: Install from the published npm package `@vacbo/oh-my-opencode`. The installed CLI binary still runs as `oh-my-opencode`. Inside `opencode.json`, the compatibility layer now prefers the plugin entry `oh-my-openagent`. Plugin config loading recognizes both `oh-my-openagent.*` and legacy `oh-my-opencode.*` basenames. If both basenames exist in the same directory, the legacy `oh-my-opencode.*` file currently wins.
...
bunx @vacbo/oh-my-opencode doctor --verbose

# Show compact system dashboard
bunx @vacbo/oh-my-opencode doctor --status

# JSON output for scripting
bunx @vacbo/oh-my-opencode doctor --json
```
...
```bash
bunx @vacbo/oh-my-opencode refresh-model-capabilities
```
...
bunx @vacbo/oh-my-opencode doctor --json > doctor-report.json
```
