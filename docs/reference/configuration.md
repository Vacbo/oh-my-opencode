...
Run `bunx @vacbo/oh-my-opencode install` for guided setup. Run `opencode models` to list available models.
...
Run `bunx @vacbo/oh-my-opencode doctor --verbose` to see effective model resolution for your config.
...
- Manual refresh is available via `bunx @vacbo/oh-my-opencode refresh-model-capabilities`.
...
| `OMO_SEND_ANONYMOUS_TELEMETRY` | Set to `1`, `true`, or `yes` to opt in to anonymous telemetry               |
| `OMO_DISABLE_POSTHOG`          | Force-disable PostHog even if telemetry is otherwise enabled                |
| `POSTHOG_API_KEY`              | Required project API key when opting in to telemetry                        |
